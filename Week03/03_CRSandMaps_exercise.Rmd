...
# starting a static map in tmap v4
tm_shape(elevation) +
  tm_raster(#col.legend = tm_legend(title ="Meters Above Sea Level"),
            palette = "-RdYlGn",
  ...
  tm_borders(col = "black", 
             lwd = 1) +
  tm_scalebar(breaks = c(0, 50, 100, 150, 200),
              text.size = 1,
              position = c("BOTTOM", "left")) + 
  tm_compass(position = c("TOP", "right"),
             type = "rose", 
             size = 2) +
  tm_credits(text = "K. Warncke, 27/2/2025") +
  tm_title("Map of Denmark's Municipalities") +
...
places <- read_sheet("https://docs.google.com/spreadsheets/d/1PlxsPElZML8LZKyXbqdAYeQCDIvDps2McZx1cTVWSzI/edit#gid=1817942479", range = "SA2025")
...
nitrates_sf <- st_as_sf(nitrates, wkt = "WKT", crs = 25832)
...
- they are not the same projection - we only see layers
...
plot(elevation);plot(nitrates_sf$WKT, add = T)
...
nitrates4326 <- st_transform(nitrates25832, crs = 4326) #from 2D to 3D crs
nitrates4326
...
plot(elevation);plot(places4326, add = T);
plot(nitrates4326$WKT, add = T)
```
...
## Your solution
```{r your-map}
#loading climate data from denmark
Den <- worldclim_country("Denmark", var="tmin", path=tempdir()) 

#filter for one month
Den_january <- subset(Den, "DNK_wc2.1_30s_tmin_1")

cols4all::c4a

#plotting it
tm_shape(Den_january) +
  tm_raster(#col.legend = tm_legend(title ="Meters Above Sea Level"),
            palette = "-RdYlGn",
            title = "Minimum temperature in Celcius Degrees",
            col.scale = tm_scale_continuous(values = "brewer.bu_gn",
                                            midpoint = 0)) +
  tm_shape(counties) +
  #tm_borders(col = "black", 
  ...
  tm_compass(position = c("TOP", "right"),
             ...
             size = 2) +
  tm_credits(text = "K. Warncke, 27/2/2025") +
  tm_title("Map of Denmark's Climate in January") +
  tm_layout(bg.color = "lightblue",
            inner.margins = c(0, 0, 0, 0))
